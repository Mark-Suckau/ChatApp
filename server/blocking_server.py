...
class TCP_Blocking_Server:
  ...
  def print_tstamp(_, msg):
    
    current_date_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print(f'[{current_date_time}] {msg}')
    
  def configure_server(self):
    self.print_tstamp('test')
    ...
    self.print_tstamp('[SERVER] Socket bound to {self.host} on port {self.port}')
    
    self.print_tstamp('[SERVER] Configured')
  ...
  def listen_for_connections(self):
    ...
    self.print_tstamp('[SERVER] Accepted new connection from address {client_addr}')
  ...
  def handle_client(self, client_sock, client_addr):
    try:
      ...
      while data_encoded:
        ...
        self.print_tstamp('[SERVER] Received {data} from {client_addr}')

        data_encoded = client_sock.recv(1024)

      self.print_tstamp('[SERVER] Client disconnected')
    except OSError as err:
      self.print_tstamp('[SERVER] Encountered error {err}')
    
    finally:
      self.print_tstamp('[SERVER] Closing client connection from {client_addr}...')
      client_sock.close()
      self.print_tstamp('[SERVER] Closed client connection from {client_addr}')
